<?php
	...
	load_plugin_textdomain( 'wp-insert-post', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );

	// Register stylesheet
	wp_register_style( 'wp-insert-post', wpip_plugin_url() . '/css/style.css', false, wpip_get_version(), 'all' );
	...
	function wpip_shortcode() {
		...
		if ( empty( $_POST['save'] ) ) {
			...
			$return .= '<div class="wpip-box"><span class="wpip-label">' . __( 'Content', 'wp-insert-post' ) . '</span><textarea name="content" rows="7" cols="70"></textarea></div>';
